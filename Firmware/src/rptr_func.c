 ...
 * 2013-10-08  CK  Dongle: local rogerbeep
 *
 * ToDo:
 * - Bug WrPos
 *
...
#define RogerBeep()	ambe_set_dtmf(AMBE_ACKTONE, 3)
...
void rptr_stopped(void) {
  ...
  // Play RogerBeep
  if (RPTR_have_all(SARPT_ACK_DGL_RBEEP|RPTR_AMBEDECODEINET)) {
    RogerBeep();
  }
}
...
int rptr_whilereceivepattern(unsigned int pattern, unsigned int bitcounter) {
  ...
  case DSTAR_SYNCSTOP:
    if (bitcounter < (24+5) ) {
      ...
      if (RPTR_is_set(RPTR_AMBEDECODEHF)) {
	// Play RogerBeep
	if (RPTR_is_set(SARPT_ACK_DGL_RBEEP)) {
	  RogerBeep();
	} else {
          ambe_standby();
	}
